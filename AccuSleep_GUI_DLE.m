...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @AccuSleep_GUI_DLE_OpeningFcn, ...
    'gui_OutputFcn',  @AccuSleep_GUI_DLE_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before AccuSleep_GUI_DLE is made visible.
function AccuSleep_GUI_DLE_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for AccuSleep_GUI_DLE
...
text(handles.axes1,.39,.496,'AccuSleep','FontSize',43,'Rotation',90,'Color',[1 .88 .42],...
    'HorizontalAlignment','center')
text(handles.axes1,.37,.504,'AccuSleep','FontSize',43,'Rotation',90,'Color',[.64 .08 .18],...
    'HorizontalAlignment','center')
text(handles.axes1,.7,.504,'Dan Lab Edition','FontSize',19,'Rotation',90,'Color',[.64 .08 .18],...
    'HorizontalAlignment','center')
...
function varargout = AccuSleep_GUI_DLE_OutputFcn(hObject, eventdata, handles)
...
lockInputs(handles,1); % lock user inputs so they don't change while this runs
...
EEGvar = whos('-file',[path,file],'EEG'); % get information about file contents
if ~isempty(EEGvar) % if we found a variable named 'EEG'
    ...
    if (length(EEGvar.size)>2 || min(EEGvar.size)~=1) || ...
        ...
        lockInputs(handles,0); % unlock the user inputs
...
lockInputs(handles,1); % lock inputs
...
message = AccuSleep_viewer_DLE(eegFile.EEG, emgFile.EMG,...
    str2num(get(handles.srBox,'String')),...
    str2num(get(handles.tsBox,'String')), labels, laser, selectedFile);
disptext(handles, message);

% complete animation
animateBoxes([ind{1}(2), ind{2}(2),ind{3}(2),ind{4}(2),ind{5}(2)], 2, codes);
lockInputs(handles,0); % unlock inputs
if ~isempty(problemString) && ~isempty(fileNames{1}) % if there was more than 1 laser file
    warndlg(problemString);
end
...
handlesToLock = {handles.recbox, handles.addrecbtn, handles.removerecbtn,...
    ...
    handles.overwriteBox, handles.boutBox, handles.loadrecbtn};
if locked
...
% --- Executes on button press in loadrecbtn.
function loadrecbtn_Callback(hObject, eventdata, handles)
% ask for dirlist
[fName,pName,~] = uigetfile('*.mat','Choose dirlist file');
% if not found, return
if ~ischar(pName)
    return
end
% try to load the dirlist
temp = load([pName,fName],'dirlist');
if ~isfield(temp,'dirlist')
    disptext(handles, 'Error: No dirlist was found in that file'); % show a helpful message
    return
else
...
if isempty(dirlist)
    disptext(handles, 'Error: dirlist is empty'); % show a helpful message
    return
end

% clear all current recordings
setappdata(handles.D,'recordings',{makeRecordingObject()});
% make sure the value attribute of the list stays in a safe range
set(handles.recbox,'Value',1);
% update the list box
set(handles.recbox,'String',{'Recording 1'});
% update our stored list of recordings
setappdata(handles.D,'recList',{'Recording 1'});
% update the display
updateDisplay(handles);

% get contents of dirlist
[problemString, fileNames] = AS_checkList(dirlist, {'EEG','EMG','labels'});

% for each entry
for i = 1:length(dirlist)
    % create new rec with as much data as possible
    x = struct;
    x.indicators = {'failure','failure','failure'}; % status of indicators (EEG,EMG,labels)
    % get length of EEG signal, and path to the file
    if isempty(fileNames{i}{1})
        x.EEGpath = ''; % path to EEG file
        x.EEGlen = [];
    else
        x.EEGpath = fileNames{i}{1}; % path to EEG file
        fileContents = whos('-file',x.EEGpath); % get information about file contents
        EEGvar = []; % just keep info about the EEG variable (if it exists)
        % look for a variable named 'EEG'
        for j = 1:length(fileContents)
            if strcmp('EEG',fileContents(j).name)
                EEGvar = fileContents(j);
                break
            end
        end
        x.EEGlen = max(EEGvar.size);
        x.indicators{1} = 'success';
    end

    % get length of EMG signal, and path to the file
    if isempty(fileNames{i}{2})
        x.EMGpath = ''; % path to EMG file
        x.EMGlen = [];
    else
        x.EMGpath = fileNames{i}{2}; % path to EMG file
        fileContents = whos('-file',x.EMGpath); % get information about file contents
        EMGvar = []; % just keep info about the EMG variable (if it exists)
        % look for a variable named 'EMG'
        for j = 1:length(fileContents)
            if strcmp('EMG',fileContents(j).name)
                EMGvar = fileContents(j);
                break
            end
        end
        x.EMGlen = max(EMGvar.size);
        x.indicators{2} = 'success';
    end
    if isempty(fileNames{i}{3})
        x.labelpath = ''; % path to label file
    else
        x.labelpath = fileNames{i}{3};
        x.indicators{3} = 'success';
    end
    if x.EEGlen ~= x.EMGlen
        x.indicators{1} = 'serious_warn';
        x.indicators{2} = 'serious_warn';
    end
    ...
    currentList = getappdata(handles.D,'recList');
    % add a new recording to the list
    currentList{i} = ['Recording ',num2str(i)];
    setappdata(handles.D,'recCountMax',i);
    % update the list box
    set(handles.recbox,'String',currentList);
    % update our stored list of recordings
    setappdata(handles.D,'recList',currentList);
    % and update our set of recording data structures
    recordings = getappdata(handles.D,'recordings');
    recordings{i} = x;
    setappdata(handles.D,'recordings',recordings);
end
% update the display
set(handles.recbox,'Value',length(dirlist));
updateDisplay(handles);
